...
# -----------------------------------------------------------------------------
fn_display_usage() {
	echo "Usage: $(basename $0) [OPTION]... <[USER@HOST:]SOURCE> <DESINATION> [exclude-pattern-file]"
	echo ""
	echo "Options"
...
fn_parse_ssh() {
	if [[ "$SRC_FOLDER" =~ ^[A-Za-z0-9\._%\+\-]+@[A-Za-z0-9.\-]+\:.+$ ]]
	then
...
if [[ -z "$DEST_FOLDER" || -z "$SRC_FOLDER" ]]; then
	fn_display_usage
	exit 1
fi
...
for ARG in "$DEST_FOLDER" "$SRC_FOLDER" "$EXCLUSION_FILE"; do
	if [[ "$ARG" == *"'"* ]]; then
...
if [ -n "$(fn_find "$INPROGRESS_FILE")" ]; then
	if [ "$OSTYPE" == "cygwin" ]; then
		...
		RUNNINGPID="$(cat $INPROGRESS_FILE)"
...
while : ; do
	...
	if [ -z "$PREVIOUS_DEST" ]; then
		...
		PREVIOUS_SRC="$(fn_get_absolute_path "$PREVIOUS_DEST")"
		fn_log_info "Previous backup found - doing incremental backup from $PREVIOUS_DEST"
		LINK_SRC_OPTION="--link-dest='$PREVIOUS_DEST'"
	fi
	...
	CMD="$CMD $LINK_SRC_OPTION"
	CMD="$CMD -- '$SSH_FOLDER_PREFIX$SRC_FOLDER/' '$DEST'"
